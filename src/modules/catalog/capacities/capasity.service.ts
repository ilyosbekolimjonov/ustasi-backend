...
export class CapasityService {
  ...
  async create(createCapasityDto: CreateCapasityDto) {
    try {
      const created = await this.prisma.capacity.create({
        data: {
  ...
  async findAll(page = 1, limit = 10, search = '') {
    try {
      ...
      const capasities = await this.prisma.capacity.findMany({
        where: {
          OR: [
  ...
  async findOne(id: string) {
    try {
      const capasity = await this.prisma.capacity.findUnique({
        where: { id },
        include: {
          tools: true,
        },
