...
export class AboutUsService {
  ...
  async create(createAboutUsDto: CreateAboutUsDto) {
    try {
      const created = await this.prisma.aboutUs.create({
        data: {
  ...
  async findAll(page = 1, limit = 10, search = '') {
    try {
      ...
      const findAll = await this.prisma.aboutUs.findMany({
        where: {
          OR: [
            {
  ...
  async findOne(id: string) {
    try {
      const findOne = await this.prisma.aboutUs.findUnique({ where: { id } });
  ...
  async remove(id: string) {
    try {
      const deleted = await this.prisma.aboutUs.delete({ where: { id } });
      return deleted;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
