...
export class ContactUsService {
  ...
  async create(createContactUsDto: CreateContactUsDto) {
    try {
      const created = await this.prisma.contactUs.create({
        data: {
  ...
  async findAll(page = 1, limit = 10, search = '') {
    try {
      ...
      const findAll = await this.prisma.contactUs.findMany({
        where: {
          OR: [
  ...
  async findOne(id: string) {
    try {
      const findOne = await this.prisma.contactUs.findUnique({ where: { id } });
  ...
  async remove(id: string) {
    try {
      const deleted = await this.prisma.contactUs.delete({ where: { id } });
      return deleted;
    } catch (error) {
      throw new InternalServerErrorException(error);
    }
