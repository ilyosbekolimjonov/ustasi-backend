...
export class OrderMasterService {
  ...
  async create(createOrderMasterDto: CreateOrderMasterDto) {
    try {
      const { orderId, masterIds } = createOrderMasterDto;
      const orderMaster = await Promise.all(
  ...
  async findAll(page = 1, limit = 10) {
    try {
      ...
      const orderMasters = await this.prisma.orderMaster.findMany({
        include: {
          master: true,
          order: true,
        },
  ...
  async findOne(id: string) {
    try {
      const orderMaster = await this.prisma.orderMaster.findUnique({
        where: { id },
        include: {
          master: true,
          order: true,
  ...
  async update(id: string, updateOrderMasterDto: UpdateOrderMasterDto) {
    try {
      const updated = await this.prisma.orderMaster.update({
  ...
  async remove(id: string) {
    try {
      const deleted = await this.prisma.orderMaster.delete({ where: { id } });
      return deleted;
    } catch (error) {
      throw new BadRequestException(error);
    }
